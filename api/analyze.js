...
module.exports = async function handler(req, res) {
  ...
  try {
    ...
    console.error('AI provider connection failed', { name: error?.name, message: error?.message });
    return res.status(502).json({
      error: 'AI provider is temporarily unavailable',
      detail: 'The server could not connect to the AI provider. Check the deployment logs for the technical cause.'
    });
  }
};
